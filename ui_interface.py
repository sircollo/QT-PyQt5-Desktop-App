# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'C:/Users/Frosted/AppData/Local/Temp/interfaceuSwNOw.ui'
#
# Created by: PyQt5 UI code generator 5.15.4
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(680, 551)
        MainWindow.setStyleSheet("*{\n"
"    border:none;\n"
"}")
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.verticalLayout = QtWidgets.QVBoxLayout(self.centralwidget)
        self.verticalLayout.setContentsMargins(0, 0, 0, 0)
        self.verticalLayout.setSpacing(0)
        self.verticalLayout.setObjectName("verticalLayout")
        self.header_frame = QtWidgets.QFrame(self.centralwidget)
        self.header_frame.setFrameShape(QtWidgets.QFrame.Box)
        self.header_frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.header_frame.setObjectName("header_frame")
        self.horizontalLayout = QtWidgets.QHBoxLayout(self.header_frame)
        self.horizontalLayout.setContentsMargins(0, 0, 0, 0)
        self.horizontalLayout.setSpacing(0)
        self.horizontalLayout.setObjectName("horizontalLayout")
        self.header_left_frame = QtWidgets.QFrame(self.header_frame)
        self.header_left_frame.setStyleSheet("padding:0;\n"
"margin:0;")
        self.header_left_frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.header_left_frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.header_left_frame.setObjectName("header_left_frame")
        self.horizontalLayout_4 = QtWidgets.QHBoxLayout(self.header_left_frame)
        self.horizontalLayout_4.setContentsMargins(0, 0, 0, 0)
        self.horizontalLayout_4.setSpacing(0)
        self.horizontalLayout_4.setObjectName("horizontalLayout_4")
        self.open_close_side_bar_btn = QtWidgets.QPushButton(self.header_left_frame)
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.open_close_side_bar_btn.setFont(font)
        self.open_close_side_bar_btn.setStyleSheet("margin:{0}")
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(":/icons/icons/align-left.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.open_close_side_bar_btn.setIcon(icon)
        self.open_close_side_bar_btn.setIconSize(QtCore.QSize(32, 32))
        self.open_close_side_bar_btn.setObjectName("open_close_side_bar_btn")
        self.horizontalLayout_4.addWidget(self.open_close_side_bar_btn, 0, QtCore.Qt.AlignLeft)
        self.horizontalLayout.addWidget(self.header_left_frame, 0, QtCore.Qt.AlignLeft)
        self.header_center_frame = QtWidgets.QFrame(self.header_frame)
        self.header_center_frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.header_center_frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.header_center_frame.setObjectName("header_center_frame")
        self.horizontalLayout_3 = QtWidgets.QHBoxLayout(self.header_center_frame)
        self.horizontalLayout_3.setObjectName("horizontalLayout_3")
        self.label = QtWidgets.QLabel(self.header_center_frame)
        font = QtGui.QFont()
        font.setPointSize(15)
        self.label.setFont(font)
        self.label.setText("")
        self.label.setPixmap(QtGui.QPixmap(":/icons/icons/airplay.svg"))
        self.label.setObjectName("label")
        self.horizontalLayout_3.addWidget(self.label, 0, QtCore.Qt.AlignRight)
        self.label_2 = QtWidgets.QLabel(self.header_center_frame)
        font = QtGui.QFont()
        font.setPointSize(17)
        font.setBold(True)
        font.setWeight(75)
        self.label_2.setFont(font)
        self.label_2.setObjectName("label_2")
        self.horizontalLayout_3.addWidget(self.label_2, 0, QtCore.Qt.AlignLeft)
        self.horizontalLayout.addWidget(self.header_center_frame)
        self.header_right_frame = QtWidgets.QFrame(self.header_frame)
        self.header_right_frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.header_right_frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.header_right_frame.setObjectName("header_right_frame")
        self.horizontalLayout_2 = QtWidgets.QHBoxLayout(self.header_right_frame)
        self.horizontalLayout_2.setContentsMargins(0, 0, 0, 0)
        self.horizontalLayout_2.setObjectName("horizontalLayout_2")
        self.minimize_window_button = QtWidgets.QPushButton(self.header_right_frame)
        self.minimize_window_button.setText("")
        icon1 = QtGui.QIcon()
        icon1.addPixmap(QtGui.QPixmap(":/icons/icons/minus.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.minimize_window_button.setIcon(icon1)
        self.minimize_window_button.setObjectName("minimize_window_button")
        self.horizontalLayout_2.addWidget(self.minimize_window_button)
        self.maximize_window_button = QtWidgets.QPushButton(self.header_right_frame)
        self.maximize_window_button.setText("")
        icon2 = QtGui.QIcon()
        icon2.addPixmap(QtGui.QPixmap(":/icons/icons/credit-card.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.maximize_window_button.setIcon(icon2)
        self.maximize_window_button.setIconSize(QtCore.QSize(16, 16))
        self.maximize_window_button.setObjectName("maximize_window_button")
        self.horizontalLayout_2.addWidget(self.maximize_window_button)
        self.close_window_button = QtWidgets.QPushButton(self.header_right_frame)
        self.close_window_button.setText("")
        icon3 = QtGui.QIcon()
        icon3.addPixmap(QtGui.QPixmap(":/icons/icons/x.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.close_window_button.setIcon(icon3)
        self.close_window_button.setIconSize(QtCore.QSize(16, 16))
        self.close_window_button.setObjectName("close_window_button")
        self.horizontalLayout_2.addWidget(self.close_window_button)
        self.horizontalLayout.addWidget(self.header_right_frame, 0, QtCore.Qt.AlignRight)
        self.verticalLayout.addWidget(self.header_frame, 0, QtCore.Qt.AlignTop)
        self.main_body_frame = QtWidgets.QFrame(self.centralwidget)
        sizePolicy = QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Expanding)
        sizePolicy.setHorizontalStretch(0)
        sizePolicy.setVerticalStretch(0)
        sizePolicy.setHeightForWidth(self.main_body_frame.sizePolicy().hasHeightForWidth())
        self.main_body_frame.setSizePolicy(sizePolicy)
        self.main_body_frame.setFrameShape(QtWidgets.QFrame.Box)
        self.main_body_frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.main_body_frame.setObjectName("main_body_frame")
        self.horizontalLayout_8 = QtWidgets.QHBoxLayout(self.main_body_frame)
        self.horizontalLayout_8.setContentsMargins(0, 0, 0, 0)
        self.horizontalLayout_8.setSpacing(0)
        self.horizontalLayout_8.setObjectName("horizontalLayout_8")
        self.left_menu_cont_frame = QtWidgets.QFrame(self.main_body_frame)
        self.left_menu_cont_frame.setMinimumSize(QtCore.QSize(41, 0))
        self.left_menu_cont_frame.setMaximumSize(QtCore.QSize(20, 16777215))
        self.left_menu_cont_frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.left_menu_cont_frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.left_menu_cont_frame.setObjectName("left_menu_cont_frame")
        self.horizontalLayout_9 = QtWidgets.QHBoxLayout(self.left_menu_cont_frame)
        self.horizontalLayout_9.setContentsMargins(0, 0, 0, 0)
        self.horizontalLayout_9.setSpacing(0)
        self.horizontalLayout_9.setObjectName("horizontalLayout_9")
        self.menu_frame = QtWidgets.QFrame(self.left_menu_cont_frame)
        self.menu_frame.setMinimumSize(QtCore.QSize(100, 0))
        self.menu_frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.menu_frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.menu_frame.setObjectName("menu_frame")
        self.gridLayout = QtWidgets.QGridLayout(self.menu_frame)
        self.gridLayout.setContentsMargins(0, 0, 0, 0)
        self.gridLayout.setSpacing(0)
        self.gridLayout.setObjectName("gridLayout")
        self.system_inf_page_btn = QtWidgets.QPushButton(self.menu_frame)
        self.system_inf_page_btn.setText("")
        icon4 = QtGui.QIcon()
        icon4.addPixmap(QtGui.QPixmap(":/icons/icons/monitor.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.system_inf_page_btn.setIcon(icon4)
        self.system_inf_page_btn.setIconSize(QtCore.QSize(32, 32))
        self.system_inf_page_btn.setObjectName("system_inf_page_btn")
        self.gridLayout.addWidget(self.system_inf_page_btn, 2, 0, 1, 1, QtCore.Qt.AlignLeft)
        self.activity_btn = QtWidgets.QPushButton(self.menu_frame)
        self.activity_btn.setText("")
        icon5 = QtGui.QIcon()
        icon5.addPixmap(QtGui.QPixmap(":/icons/icons/activity.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.activity_btn.setIcon(icon5)
        self.activity_btn.setIconSize(QtCore.QSize(32, 32))
        self.activity_btn.setObjectName("activity_btn")
        self.gridLayout.addWidget(self.activity_btn, 3, 0, 1, 1, QtCore.Qt.AlignLeft)
        self.battery_page_btn = QtWidgets.QPushButton(self.menu_frame)
        self.battery_page_btn.setText("")
        icon6 = QtGui.QIcon()
        icon6.addPixmap(QtGui.QPixmap(":/icons/icons/battery-charging.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.battery_page_btn.setIcon(icon6)
        self.battery_page_btn.setIconSize(QtCore.QSize(32, 32))
        self.battery_page_btn.setObjectName("battery_page_btn")
        self.gridLayout.addWidget(self.battery_page_btn, 1, 0, 1, 1, QtCore.Qt.AlignLeft)
        self.cpu_page_btn = QtWidgets.QPushButton(self.menu_frame)
        self.cpu_page_btn.setAutoFillBackground(False)
        self.cpu_page_btn.setText("")
        icon7 = QtGui.QIcon()
        icon7.addPixmap(QtGui.QPixmap(":/icons/icons/cpu.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.cpu_page_btn.setIcon(icon7)
        self.cpu_page_btn.setIconSize(QtCore.QSize(32, 32))
        self.cpu_page_btn.setObjectName("cpu_page_btn")
        self.gridLayout.addWidget(self.cpu_page_btn, 0, 0, 1, 1, QtCore.Qt.AlignLeft)
        self.label_4 = QtWidgets.QLabel(self.menu_frame)
        self.label_4.setObjectName("label_4")
        self.gridLayout.addWidget(self.label_4, 0, 1, 1, 1, QtCore.Qt.AlignLeft)
        self.storage_btn = QtWidgets.QPushButton(self.menu_frame)
        self.storage_btn.setText("")
        icon8 = QtGui.QIcon()
        icon8.addPixmap(QtGui.QPixmap(":/icons/icons/disc.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.storage_btn.setIcon(icon8)
        self.storage_btn.setIconSize(QtCore.QSize(32, 32))
        self.storage_btn.setObjectName("storage_btn")
        self.gridLayout.addWidget(self.storage_btn, 4, 0, 1, 1, QtCore.Qt.AlignLeft)
        self.sensors_page_btn = QtWidgets.QPushButton(self.menu_frame)
        self.sensors_page_btn.setText("")
        icon9 = QtGui.QIcon()
        icon9.addPixmap(QtGui.QPixmap(":/icons/icons/thermometer.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.sensors_page_btn.setIcon(icon9)
        self.sensors_page_btn.setIconSize(QtCore.QSize(32, 32))
        self.sensors_page_btn.setObjectName("sensors_page_btn")
        self.gridLayout.addWidget(self.sensors_page_btn, 5, 0, 1, 1, QtCore.Qt.AlignLeft)
        self.networks_page_button = QtWidgets.QPushButton(self.menu_frame)
        self.networks_page_button.setText("")
        icon10 = QtGui.QIcon()
        icon10.addPixmap(QtGui.QPixmap(":/icons/icons/wifi.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.networks_page_button.setIcon(icon10)
        self.networks_page_button.setIconSize(QtCore.QSize(32, 32))
        self.networks_page_button.setObjectName("networks_page_button")
        self.gridLayout.addWidget(self.networks_page_button, 6, 0, 1, 1, QtCore.Qt.AlignLeft)
        self.label_5 = QtWidgets.QLabel(self.menu_frame)
        self.label_5.setObjectName("label_5")
        self.gridLayout.addWidget(self.label_5, 1, 1, 1, 1, QtCore.Qt.AlignLeft)
        self.label_6 = QtWidgets.QLabel(self.menu_frame)
        self.label_6.setObjectName("label_6")
        self.gridLayout.addWidget(self.label_6, 2, 1, 1, 1, QtCore.Qt.AlignLeft)
        self.label_7 = QtWidgets.QLabel(self.menu_frame)
        self.label_7.setObjectName("label_7")
        self.gridLayout.addWidget(self.label_7, 3, 1, 1, 1)
        self.label_8 = QtWidgets.QLabel(self.menu_frame)
        self.label_8.setObjectName("label_8")
        self.gridLayout.addWidget(self.label_8, 4, 1, 1, 1, QtCore.Qt.AlignLeft)
        self.label_9 = QtWidgets.QLabel(self.menu_frame)
        self.label_9.setObjectName("label_9")
        self.gridLayout.addWidget(self.label_9, 5, 1, 1, 1)
        self.label_10 = QtWidgets.QLabel(self.menu_frame)
        self.label_10.setObjectName("label_10")
        self.gridLayout.addWidget(self.label_10, 6, 1, 1, 1)
        self.horizontalLayout_9.addWidget(self.menu_frame, 0, QtCore.Qt.AlignLeft|QtCore.Qt.AlignTop)
        self.horizontalLayout_8.addWidget(self.left_menu_cont_frame)
        self.main_body_contents = QtWidgets.QFrame(self.main_body_frame)
        self.main_body_contents.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.main_body_contents.setFrameShadow(QtWidgets.QFrame.Raised)
        self.main_body_contents.setObjectName("main_body_contents")
        self.verticalLayout_4 = QtWidgets.QVBoxLayout(self.main_body_contents)
        self.verticalLayout_4.setContentsMargins(0, 0, 0, 0)
        self.verticalLayout_4.setSpacing(0)
        self.verticalLayout_4.setObjectName("verticalLayout_4")
        self.stackedWidget = QtWidgets.QStackedWidget(self.main_body_contents)
        self.stackedWidget.setObjectName("stackedWidget")
        self.cpu_and_memory = QtWidgets.QWidget()
        self.cpu_and_memory.setObjectName("cpu_and_memory")
        self.verticalLayout_5 = QtWidgets.QVBoxLayout(self.cpu_and_memory)
        self.verticalLayout_5.setObjectName("verticalLayout_5")
        self.cpu_info_frame = QtWidgets.QFrame(self.cpu_and_memory)
        self.cpu_info_frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.cpu_info_frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.cpu_info_frame.setObjectName("cpu_info_frame")
        self.gridLayout_2 = QtWidgets.QGridLayout(self.cpu_info_frame)
        self.gridLayout_2.setObjectName("gridLayout_2")
        self.label_16 = QtWidgets.QLabel(self.cpu_info_frame)
        self.label_16.setObjectName("label_16")
        self.gridLayout_2.addWidget(self.label_16, 0, 0, 1, 1)
        self.cpu_count = QtWidgets.QLabel(self.cpu_info_frame)
        self.cpu_count.setObjectName("cpu_count")
        self.gridLayout_2.addWidget(self.cpu_count, 0, 1, 1, 1)
        self.cpu_per = QtWidgets.QLabel(self.cpu_info_frame)
        self.cpu_per.setObjectName("cpu_per")
        self.gridLayout_2.addWidget(self.cpu_per, 1, 1, 1, 1)
        self.label_20 = QtWidgets.QLabel(self.cpu_info_frame)
        self.label_20.setObjectName("label_20")
        self.gridLayout_2.addWidget(self.label_20, 2, 0, 1, 1)
        self.cpu_main_core = QtWidgets.QLabel(self.cpu_info_frame)
        self.cpu_main_core.setObjectName("cpu_main_core")
        self.gridLayout_2.addWidget(self.cpu_main_core, 2, 1, 1, 1)
        self.label_18 = QtWidgets.QLabel(self.cpu_info_frame)
        self.label_18.setObjectName("label_18")
        self.gridLayout_2.addWidget(self.label_18, 1, 0, 1, 1)
        self.verticalLayout_5.addWidget(self.cpu_info_frame)
        self.ram_info_frame = QtWidgets.QFrame(self.cpu_and_memory)
        self.ram_info_frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.ram_info_frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.ram_info_frame.setObjectName("ram_info_frame")
        self.gridLayout_3 = QtWidgets.QGridLayout(self.ram_info_frame)
        self.gridLayout_3.setObjectName("gridLayout_3")
        self.label_26 = QtWidgets.QLabel(self.ram_info_frame)
        self.label_26.setObjectName("label_26")
        self.gridLayout_3.addWidget(self.label_26, 2, 0, 1, 1)
        self.available_ram = QtWidgets.QLabel(self.ram_info_frame)
        self.available_ram.setObjectName("available_ram")
        self.gridLayout_3.addWidget(self.available_ram, 1, 1, 1, 1)
        self.used_ram = QtWidgets.QLabel(self.ram_info_frame)
        self.used_ram.setObjectName("used_ram")
        self.gridLayout_3.addWidget(self.used_ram, 2, 1, 1, 1)
        self.label_24 = QtWidgets.QLabel(self.ram_info_frame)
        self.label_24.setObjectName("label_24")
        self.gridLayout_3.addWidget(self.label_24, 1, 0, 1, 1)
        self.total_ram = QtWidgets.QLabel(self.ram_info_frame)
        self.total_ram.setObjectName("total_ram")
        self.gridLayout_3.addWidget(self.total_ram, 0, 1, 1, 1)
        self.label_22 = QtWidgets.QLabel(self.ram_info_frame)
        self.label_22.setObjectName("label_22")
        self.gridLayout_3.addWidget(self.label_22, 0, 0, 1, 1)
        self.free_ram = QtWidgets.QLabel(self.ram_info_frame)
        self.free_ram.setObjectName("free_ram")
        self.gridLayout_3.addWidget(self.free_ram, 3, 1, 1, 1)
        self.label_30 = QtWidgets.QLabel(self.ram_info_frame)
        self.label_30.setObjectName("label_30")
        self.gridLayout_3.addWidget(self.label_30, 4, 0, 1, 1)
        self.ram_usage = QtWidgets.QLabel(self.ram_info_frame)
        self.ram_usage.setObjectName("ram_usage")
        self.gridLayout_3.addWidget(self.ram_usage, 4, 1, 1, 1)
        self.label_28 = QtWidgets.QLabel(self.ram_info_frame)
        self.label_28.setObjectName("label_28")
        self.gridLayout_3.addWidget(self.label_28, 3, 0, 1, 1)
        self.verticalLayout_5.addWidget(self.ram_info_frame)
        self.stackedWidget.addWidget(self.cpu_and_memory)
        self.battery = QtWidgets.QWidget()
        self.battery.setObjectName("battery")
        self.verticalLayout_6 = QtWidgets.QVBoxLayout(self.battery)
        self.verticalLayout_6.setObjectName("verticalLayout_6")
        self.label_32 = QtWidgets.QLabel(self.battery)
        font = QtGui.QFont()
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.label_32.setFont(font)
        self.label_32.setObjectName("label_32")
        self.verticalLayout_6.addWidget(self.label_32, 0, QtCore.Qt.AlignBottom)
        self.frame_6 = QtWidgets.QFrame(self.battery)
        self.frame_6.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_6.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_6.setObjectName("frame_6")
        self.gridLayout_6 = QtWidgets.QGridLayout(self.frame_6)
        self.gridLayout_6.setObjectName("gridLayout_6")
        self.label_33 = QtWidgets.QLabel(self.frame_6)
        self.label_33.setObjectName("label_33")
        self.gridLayout_6.addWidget(self.label_33, 0, 0, 1, 1)
        self.battery_status = QtWidgets.QLabel(self.frame_6)
        self.battery_status.setObjectName("battery_status")
        self.gridLayout_6.addWidget(self.battery_status, 0, 1, 1, 1)
        self.label_34 = QtWidgets.QLabel(self.frame_6)
        self.label_34.setObjectName("label_34")
        self.gridLayout_6.addWidget(self.label_34, 1, 0, 1, 1)
        self.battery_charge = QtWidgets.QLabel(self.frame_6)
        self.battery_charge.setObjectName("battery_charge")
        self.gridLayout_6.addWidget(self.battery_charge, 1, 1, 1, 1)
        self.label_35 = QtWidgets.QLabel(self.frame_6)
        self.label_35.setObjectName("label_35")
        self.gridLayout_6.addWidget(self.label_35, 2, 0, 1, 1)
        self.battery_time_left = QtWidgets.QLabel(self.frame_6)
        self.battery_time_left.setObjectName("battery_time_left")
        self.gridLayout_6.addWidget(self.battery_time_left, 2, 1, 1, 1)
        self.label_36 = QtWidgets.QLabel(self.frame_6)
        self.label_36.setObjectName("label_36")
        self.gridLayout_6.addWidget(self.label_36, 3, 0, 1, 1)
        self.battery_plugged = QtWidgets.QLabel(self.frame_6)
        self.battery_plugged.setObjectName("battery_plugged")
        self.gridLayout_6.addWidget(self.battery_plugged, 3, 1, 1, 1)
        self.verticalLayout_6.addWidget(self.frame_6)
        self.stackedWidget.addWidget(self.battery)
        self.activities = QtWidgets.QWidget()
        self.activities.setObjectName("activities")
        self.verticalLayout_8 = QtWidgets.QVBoxLayout(self.activities)
        self.verticalLayout_8.setObjectName("verticalLayout_8")
        self.frame_8 = QtWidgets.QFrame(self.activities)
        self.frame_8.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_8.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_8.setObjectName("frame_8")
        self.horizontalLayout_11 = QtWidgets.QHBoxLayout(self.frame_8)
        self.horizontalLayout_11.setObjectName("horizontalLayout_11")
        self.label_55 = QtWidgets.QLabel(self.frame_8)
        font = QtGui.QFont()
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.label_55.setFont(font)
        self.label_55.setObjectName("label_55")
        self.horizontalLayout_11.addWidget(self.label_55)
        self.frame_11 = QtWidgets.QFrame(self.frame_8)
        self.frame_11.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_11.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_11.setObjectName("frame_11")
        self.horizontalLayout_12 = QtWidgets.QHBoxLayout(self.frame_11)
        self.horizontalLayout_12.setObjectName("horizontalLayout_12")
        self.activity_search = QtWidgets.QLineEdit(self.frame_11)
        self.activity_search.setObjectName("activity_search")
        self.horizontalLayout_12.addWidget(self.activity_search, 0, QtCore.Qt.AlignRight)
        self.pushButton_6 = QtWidgets.QPushButton(self.frame_11)
        self.pushButton_6.setText("")
        icon11 = QtGui.QIcon()
        icon11.addPixmap(QtGui.QPixmap(":/icons/icons/search.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.pushButton_6.setIcon(icon11)
        self.pushButton_6.setObjectName("pushButton_6")
        self.horizontalLayout_12.addWidget(self.pushButton_6)
        self.horizontalLayout_11.addWidget(self.frame_11)
        self.verticalLayout_8.addWidget(self.frame_8, 0, QtCore.Qt.AlignTop)
        self.frame_9 = QtWidgets.QFrame(self.activities)
        self.frame_9.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_9.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_9.setObjectName("frame_9")
        self.verticalLayout_9 = QtWidgets.QVBoxLayout(self.frame_9)
        self.verticalLayout_9.setObjectName("verticalLayout_9")
        self.tableWidget = QtWidgets.QTableWidget(self.frame_9)
        self.tableWidget.setObjectName("tableWidget")
        self.tableWidget.setColumnCount(8)
        self.tableWidget.setRowCount(0)
        item = QtWidgets.QTableWidgetItem()
        self.tableWidget.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.tableWidget.setHorizontalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.tableWidget.setHorizontalHeaderItem(2, item)
        item = QtWidgets.QTableWidgetItem()
        self.tableWidget.setHorizontalHeaderItem(3, item)
        item = QtWidgets.QTableWidgetItem()
        self.tableWidget.setHorizontalHeaderItem(4, item)
        item = QtWidgets.QTableWidgetItem()
        self.tableWidget.setHorizontalHeaderItem(5, item)
        item = QtWidgets.QTableWidgetItem()
        self.tableWidget.setHorizontalHeaderItem(6, item)
        item = QtWidgets.QTableWidgetItem()
        self.tableWidget.setHorizontalHeaderItem(7, item)
        self.verticalLayout_9.addWidget(self.tableWidget)
        self.verticalLayout_8.addWidget(self.frame_9)
        self.frame_10 = QtWidgets.QFrame(self.activities)
        self.frame_10.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_10.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_10.setObjectName("frame_10")
        self.horizontalLayout_13 = QtWidgets.QHBoxLayout(self.frame_10)
        self.horizontalLayout_13.setObjectName("horizontalLayout_13")
        self.pushButton_7 = QtWidgets.QPushButton(self.frame_10)
        self.pushButton_7.setObjectName("pushButton_7")
        self.horizontalLayout_13.addWidget(self.pushButton_7)
        self.pushButton_8 = QtWidgets.QPushButton(self.frame_10)
        self.pushButton_8.setObjectName("pushButton_8")
        self.horizontalLayout_13.addWidget(self.pushButton_8)
        self.pushButton_9 = QtWidgets.QPushButton(self.frame_10)
        self.pushButton_9.setObjectName("pushButton_9")
        self.horizontalLayout_13.addWidget(self.pushButton_9)
        self.pushButton_10 = QtWidgets.QPushButton(self.frame_10)
        self.pushButton_10.setObjectName("pushButton_10")
        self.horizontalLayout_13.addWidget(self.pushButton_10)
        self.verticalLayout_8.addWidget(self.frame_10, 0, QtCore.Qt.AlignBottom)
        self.stackedWidget.addWidget(self.activities)
        self.storage = QtWidgets.QWidget()
        self.storage.setObjectName("storage")
        self.verticalLayout_10 = QtWidgets.QVBoxLayout(self.storage)
        self.verticalLayout_10.setObjectName("verticalLayout_10")
        self.label_56 = QtWidgets.QLabel(self.storage)
        font = QtGui.QFont()
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.label_56.setFont(font)
        self.label_56.setObjectName("label_56")
        self.verticalLayout_10.addWidget(self.label_56)
        self.storageTable = QtWidgets.QTableWidget(self.storage)
        self.storageTable.setObjectName("storageTable")
        self.storageTable.setColumnCount(9)
        self.storageTable.setRowCount(0)
        item = QtWidgets.QTableWidgetItem()
        self.storageTable.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.storageTable.setHorizontalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.storageTable.setHorizontalHeaderItem(2, item)
        item = QtWidgets.QTableWidgetItem()
        self.storageTable.setHorizontalHeaderItem(3, item)
        item = QtWidgets.QTableWidgetItem()
        self.storageTable.setHorizontalHeaderItem(4, item)
        item = QtWidgets.QTableWidgetItem()
        self.storageTable.setHorizontalHeaderItem(5, item)
        item = QtWidgets.QTableWidgetItem()
        self.storageTable.setHorizontalHeaderItem(6, item)
        item = QtWidgets.QTableWidgetItem()
        self.storageTable.setHorizontalHeaderItem(7, item)
        item = QtWidgets.QTableWidgetItem()
        self.storageTable.setHorizontalHeaderItem(8, item)
        self.verticalLayout_10.addWidget(self.storageTable)
        self.stackedWidget.addWidget(self.storage)
        self.sensors = QtWidgets.QWidget()
        self.sensors.setObjectName("sensors")
        self.verticalLayout_11 = QtWidgets.QVBoxLayout(self.sensors)
        self.verticalLayout_11.setObjectName("verticalLayout_11")
        self.label_57 = QtWidgets.QLabel(self.sensors)
        font = QtGui.QFont()
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.label_57.setFont(font)
        self.label_57.setObjectName("label_57")
        self.verticalLayout_11.addWidget(self.label_57)
        self.sensorTable = QtWidgets.QTableWidget(self.sensors)
        self.sensorTable.setObjectName("sensorTable")
        self.sensorTable.setColumnCount(5)
        self.sensorTable.setRowCount(0)
        item = QtWidgets.QTableWidgetItem()
        self.sensorTable.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.sensorTable.setHorizontalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.sensorTable.setHorizontalHeaderItem(2, item)
        item = QtWidgets.QTableWidgetItem()
        self.sensorTable.setHorizontalHeaderItem(3, item)
        item = QtWidgets.QTableWidgetItem()
        self.sensorTable.setHorizontalHeaderItem(4, item)
        self.verticalLayout_11.addWidget(self.sensorTable)
        self.stackedWidget.addWidget(self.sensors)
        self.networks = QtWidgets.QWidget()
        self.networks.setObjectName("networks")
        self.verticalLayout_12 = QtWidgets.QVBoxLayout(self.networks)
        self.verticalLayout_12.setObjectName("verticalLayout_12")
        self.scrollArea = QtWidgets.QScrollArea(self.networks)
        self.scrollArea.setWidgetResizable(True)
        self.scrollArea.setObjectName("scrollArea")
        self.scrollAreaWidgetContents = QtWidgets.QWidget()
        self.scrollAreaWidgetContents.setGeometry(QtCore.QRect(0, 0, 373, 456))
        self.scrollAreaWidgetContents.setObjectName("scrollAreaWidgetContents")
        self.verticalLayout_13 = QtWidgets.QVBoxLayout(self.scrollAreaWidgetContents)
        self.verticalLayout_13.setContentsMargins(0, 0, 0, 0)
        self.verticalLayout_13.setSpacing(0)
        self.verticalLayout_13.setObjectName("verticalLayout_13")
        self.frame_5 = QtWidgets.QFrame(self.scrollAreaWidgetContents)
        self.frame_5.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_5.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_5.setObjectName("frame_5")
        self.verticalLayout_14 = QtWidgets.QVBoxLayout(self.frame_5)
        self.verticalLayout_14.setObjectName("verticalLayout_14")
        self.label_58 = QtWidgets.QLabel(self.frame_5)
        font = QtGui.QFont()
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.label_58.setFont(font)
        self.label_58.setObjectName("label_58")
        self.verticalLayout_14.addWidget(self.label_58)
        self.net_stats_table = QtWidgets.QTableWidget(self.frame_5)
        self.net_stats_table.setObjectName("net_stats_table")
        self.net_stats_table.setColumnCount(5)
        self.net_stats_table.setRowCount(0)
        item = QtWidgets.QTableWidgetItem()
        self.net_stats_table.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_stats_table.setHorizontalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_stats_table.setHorizontalHeaderItem(2, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_stats_table.setHorizontalHeaderItem(3, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_stats_table.setHorizontalHeaderItem(4, item)
        self.verticalLayout_14.addWidget(self.net_stats_table)
        self.verticalLayout_13.addWidget(self.frame_5)
        self.frame_12 = QtWidgets.QFrame(self.scrollAreaWidgetContents)
        self.frame_12.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_12.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_12.setObjectName("frame_12")
        self.verticalLayout_15 = QtWidgets.QVBoxLayout(self.frame_12)
        self.verticalLayout_15.setObjectName("verticalLayout_15")
        self.label_59 = QtWidgets.QLabel(self.frame_12)
        font = QtGui.QFont()
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.label_59.setFont(font)
        self.label_59.setObjectName("label_59")
        self.verticalLayout_15.addWidget(self.label_59)
        self.net_io_table = QtWidgets.QTableWidget(self.frame_12)
        self.net_io_table.setObjectName("net_io_table")
        self.net_io_table.setColumnCount(9)
        self.net_io_table.setRowCount(0)
        item = QtWidgets.QTableWidgetItem()
        self.net_io_table.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_io_table.setHorizontalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_io_table.setHorizontalHeaderItem(2, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_io_table.setHorizontalHeaderItem(3, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_io_table.setHorizontalHeaderItem(4, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_io_table.setHorizontalHeaderItem(5, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_io_table.setHorizontalHeaderItem(6, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_io_table.setHorizontalHeaderItem(7, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_io_table.setHorizontalHeaderItem(8, item)
        self.verticalLayout_15.addWidget(self.net_io_table)
        self.verticalLayout_13.addWidget(self.frame_12)
        self.frame_13 = QtWidgets.QFrame(self.scrollAreaWidgetContents)
        self.frame_13.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_13.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_13.setObjectName("frame_13")
        self.verticalLayout_16 = QtWidgets.QVBoxLayout(self.frame_13)
        self.verticalLayout_16.setObjectName("verticalLayout_16")
        self.label_60 = QtWidgets.QLabel(self.frame_13)
        font = QtGui.QFont()
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.label_60.setFont(font)
        self.label_60.setObjectName("label_60")
        self.verticalLayout_16.addWidget(self.label_60)
        self.net_addresses_table = QtWidgets.QTableWidget(self.frame_13)
        self.net_addresses_table.setObjectName("net_addresses_table")
        self.net_addresses_table.setColumnCount(6)
        self.net_addresses_table.setRowCount(0)
        item = QtWidgets.QTableWidgetItem()
        self.net_addresses_table.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_addresses_table.setHorizontalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_addresses_table.setHorizontalHeaderItem(2, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_addresses_table.setHorizontalHeaderItem(3, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_addresses_table.setHorizontalHeaderItem(4, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_addresses_table.setHorizontalHeaderItem(5, item)
        self.verticalLayout_16.addWidget(self.net_addresses_table)
        self.verticalLayout_13.addWidget(self.frame_13)
        self.frame_14 = QtWidgets.QFrame(self.scrollAreaWidgetContents)
        self.frame_14.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_14.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_14.setObjectName("frame_14")
        self.verticalLayout_17 = QtWidgets.QVBoxLayout(self.frame_14)
        self.verticalLayout_17.setObjectName("verticalLayout_17")
        self.label_61 = QtWidgets.QLabel(self.frame_14)
        font = QtGui.QFont()
        font.setPointSize(12)
        font.setBold(True)
        font.setWeight(75)
        self.label_61.setFont(font)
        self.label_61.setObjectName("label_61")
        self.verticalLayout_17.addWidget(self.label_61)
        self.net_connections_table = QtWidgets.QTableWidget(self.frame_14)
        self.net_connections_table.setObjectName("net_connections_table")
        self.net_connections_table.setColumnCount(6)
        self.net_connections_table.setRowCount(0)
        item = QtWidgets.QTableWidgetItem()
        self.net_connections_table.setHorizontalHeaderItem(0, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_connections_table.setHorizontalHeaderItem(1, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_connections_table.setHorizontalHeaderItem(2, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_connections_table.setHorizontalHeaderItem(3, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_connections_table.setHorizontalHeaderItem(4, item)
        item = QtWidgets.QTableWidgetItem()
        self.net_connections_table.setHorizontalHeaderItem(5, item)
        self.verticalLayout_17.addWidget(self.net_connections_table)
        self.verticalLayout_13.addWidget(self.frame_14)
        self.scrollArea.setWidget(self.scrollAreaWidgetContents)
        self.verticalLayout_12.addWidget(self.scrollArea)
        self.stackedWidget.addWidget(self.networks)
        self.system_info = QtWidgets.QWidget()
        self.system_info.setObjectName("system_info")
        self.verticalLayout_7 = QtWidgets.QVBoxLayout(self.system_info)
        self.verticalLayout_7.setObjectName("verticalLayout_7")
        self.frame_7 = QtWidgets.QFrame(self.system_info)
        self.frame_7.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_7.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_7.setObjectName("frame_7")
        self.gridLayout_4 = QtWidgets.QGridLayout(self.frame_7)
        self.gridLayout_4.setObjectName("gridLayout_4")
        self.label_44 = QtWidgets.QLabel(self.frame_7)
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.label_44.setFont(font)
        self.label_44.setObjectName("label_44")
        self.gridLayout_4.addWidget(self.label_44, 4, 0, 1, 1)
        self.label_43 = QtWidgets.QLabel(self.frame_7)
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.label_43.setFont(font)
        self.label_43.setObjectName("label_43")
        self.gridLayout_4.addWidget(self.label_43, 3, 0, 1, 1)
        self.label_41 = QtWidgets.QLabel(self.frame_7)
        font = QtGui.QFont()
        font.setPointSize(12)
        self.label_41.setFont(font)
        self.label_41.setObjectName("label_41")
        self.gridLayout_4.addWidget(self.label_41, 0, 0, 1, 1, QtCore.Qt.AlignTop)
        self.system_processor = QtWidgets.QLabel(self.frame_7)
        self.system_processor.setObjectName("system_processor")
        self.gridLayout_4.addWidget(self.system_processor, 3, 3, 1, 1)
        self.system_platform = QtWidgets.QLabel(self.frame_7)
        self.system_platform.setObjectName("system_platform")
        self.gridLayout_4.addWidget(self.system_platform, 3, 1, 1, 1)
        self.system_version = QtWidgets.QLabel(self.frame_7)
        self.system_version.setObjectName("system_version")
        self.gridLayout_4.addWidget(self.system_version, 4, 1, 1, 1)
        self.label_45 = QtWidgets.QLabel(self.frame_7)
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.label_45.setFont(font)
        self.label_45.setObjectName("label_45")
        self.gridLayout_4.addWidget(self.label_45, 5, 0, 1, 1)
        self.system_date = QtWidgets.QLabel(self.frame_7)
        self.system_date.setObjectName("system_date")
        self.gridLayout_4.addWidget(self.system_date, 5, 1, 1, 1)
        self.label_51 = QtWidgets.QLabel(self.frame_7)
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.label_51.setFont(font)
        self.label_51.setObjectName("label_51")
        self.gridLayout_4.addWidget(self.label_51, 5, 2, 1, 1)
        self.label_49 = QtWidgets.QLabel(self.frame_7)
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.label_49.setFont(font)
        self.label_49.setObjectName("label_49")
        self.gridLayout_4.addWidget(self.label_49, 3, 2, 1, 1)
        self.label_50 = QtWidgets.QLabel(self.frame_7)
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.label_50.setFont(font)
        self.label_50.setObjectName("label_50")
        self.gridLayout_4.addWidget(self.label_50, 4, 2, 1, 1)
        self.system_time = QtWidgets.QLabel(self.frame_7)
        self.system_time.setObjectName("system_time")
        self.gridLayout_4.addWidget(self.system_time, 5, 3, 1, 1)
        self.system_machine = QtWidgets.QLabel(self.frame_7)
        self.system_machine.setObjectName("system_machine")
        self.gridLayout_4.addWidget(self.system_machine, 4, 3, 1, 1)
        self.system_system = QtWidgets.QLabel(self.frame_7)
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        font.setWeight(75)
        self.system_system.setFont(font)
        self.system_system.setObjectName("system_system")
        self.gridLayout_4.addWidget(self.system_system, 2, 0, 1, 1)
        self.verticalLayout_7.addWidget(self.frame_7)
        self.stackedWidget.addWidget(self.system_info)
        self.verticalLayout_4.addWidget(self.stackedWidget)
        self.horizontalLayout_8.addWidget(self.main_body_contents)
        self.frame_4 = QtWidgets.QFrame(self.main_body_frame)
        self.frame_4.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_4.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_4.setObjectName("frame_4")
        self.verticalLayout_2 = QtWidgets.QVBoxLayout(self.frame_4)
        self.verticalLayout_2.setObjectName("verticalLayout_2")
        self.frame = QtWidgets.QFrame(self.frame_4)
        self.frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame.setObjectName("frame")
        self.verticalLayout_3 = QtWidgets.QVBoxLayout(self.frame)
        self.verticalLayout_3.setObjectName("verticalLayout_3")
        self.label_11 = QtWidgets.QLabel(self.frame)
        self.label_11.setObjectName("label_11")
        self.verticalLayout_3.addWidget(self.label_11, 0, QtCore.Qt.AlignLeft|QtCore.Qt.AlignTop)
        self.label_12 = QtWidgets.QLabel(self.frame)
        self.label_12.setObjectName("label_12")
        self.verticalLayout_3.addWidget(self.label_12, 0, QtCore.Qt.AlignLeft|QtCore.Qt.AlignTop)
        self.verticalLayout_2.addWidget(self.frame)
        self.frame_3 = QtWidgets.QFrame(self.frame_4)
        self.frame_3.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_3.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_3.setObjectName("frame_3")
        self.gridLayout_5 = QtWidgets.QGridLayout(self.frame_3)
        self.gridLayout_5.setObjectName("gridLayout_5")
        self.pushButton_4 = QtWidgets.QPushButton(self.frame_3)
        self.pushButton_4.setText("")
        icon12 = QtGui.QIcon()
        icon12.addPixmap(QtGui.QPixmap(":/icons/icons/coffee.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.pushButton_4.setIcon(icon12)
        self.pushButton_4.setIconSize(QtCore.QSize(32, 32))
        self.pushButton_4.setObjectName("pushButton_4")
        self.gridLayout_5.addWidget(self.pushButton_4, 2, 0, 1, 1)
        self.pushButton_5 = QtWidgets.QPushButton(self.frame_3)
        self.pushButton_5.setText("")
        icon13 = QtGui.QIcon()
        icon13.addPixmap(QtGui.QPixmap(":/icons/icons/globe.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.pushButton_5.setIcon(icon13)
        self.pushButton_5.setIconSize(QtCore.QSize(32, 32))
        self.pushButton_5.setObjectName("pushButton_5")
        self.gridLayout_5.addWidget(self.pushButton_5, 3, 0, 1, 1)
        self.pushButton_3 = QtWidgets.QPushButton(self.frame_3)
        self.pushButton_3.setText("")
        icon14 = QtGui.QIcon()
        icon14.addPixmap(QtGui.QPixmap(":/icons/icons/youtube.svg"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        self.pushButton_3.setIcon(icon14)
        self.pushButton_3.setIconSize(QtCore.QSize(32, 32))
        self.pushButton_3.setObjectName("pushButton_3")
        self.gridLayout_5.addWidget(self.pushButton_3, 1, 0, 1, 1)
        self.label_13 = QtWidgets.QLabel(self.frame_3)
        font = QtGui.QFont()
        font.setUnderline(True)
        self.label_13.setFont(font)
        self.label_13.setObjectName("label_13")
        self.gridLayout_5.addWidget(self.label_13, 1, 1, 1, 1)
        self.label_14 = QtWidgets.QLabel(self.frame_3)
        font = QtGui.QFont()
        font.setUnderline(True)
        self.label_14.setFont(font)
        self.label_14.setObjectName("label_14")
        self.gridLayout_5.addWidget(self.label_14, 2, 1, 1, 1)
        self.label_15 = QtWidgets.QLabel(self.frame_3)
        font = QtGui.QFont()
        font.setUnderline(True)
        self.label_15.setFont(font)
        self.label_15.setObjectName("label_15")
        self.gridLayout_5.addWidget(self.label_15, 3, 1, 1, 1)
        self.verticalLayout_2.addWidget(self.frame_3, 0, QtCore.Qt.AlignTop)
        self.horizontalLayout_8.addWidget(self.frame_4)
        self.verticalLayout.addWidget(self.main_body_frame)
        self.footer_frame = QtWidgets.QFrame(self.centralwidget)
        self.footer_frame.setFrameShape(QtWidgets.QFrame.Box)
        self.footer_frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.footer_frame.setObjectName("footer_frame")
        self.horizontalLayout_5 = QtWidgets.QHBoxLayout(self.footer_frame)
        self.horizontalLayout_5.setContentsMargins(0, 0, 0, 0)
        self.horizontalLayout_5.setSpacing(0)
        self.horizontalLayout_5.setObjectName("horizontalLayout_5")
        self.footer_left_frame = QtWidgets.QFrame(self.footer_frame)
        self.footer_left_frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.footer_left_frame.setFrameShadow(QtWidgets.QFrame.Raised)
        self.footer_left_frame.setObjectName("footer_left_frame")
        self.horizontalLayout_6 = QtWidgets.QHBoxLayout(self.footer_left_frame)
        self.horizontalLayout_6.setObjectName("horizontalLayout_6")
        self.label_3 = QtWidgets.QLabel(self.footer_left_frame)
        self.label_3.setObjectName("label_3")
        self.horizontalLayout_6.addWidget(self.label_3, 0, QtCore.Qt.AlignLeft)
        self.horizontalLayout_5.addWidget(self.footer_left_frame)
        self.frame_2 = QtWidgets.QFrame(self.footer_frame)
        self.frame_2.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.frame_2.setFrameShadow(QtWidgets.QFrame.Raised)
        self.frame_2.setObjectName("frame_2")
        self.horizontalLayout_7 = QtWidgets.QHBoxLayout(self.frame_2)
        self.horizontalLayout_7.setObjectName("horizontalLayout_7")
        self.pushButton_2 = QtWidgets.QPushButton(self.frame_2)
        self.pushButton_2.setObjectName("pushButton_2")
        self.horizontalLayout_7.addWidget(self.pushButton_2, 0, QtCore.Qt.AlignRight)
        self.size_grip = QtWidgets.QFrame(self.frame_2)
        self.size_grip.setMinimumSize(QtCore.QSize(10, 10))
        self.size_grip.setMaximumSize(QtCore.QSize(10, 10))
        self.size_grip.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.size_grip.setFrameShadow(QtWidgets.QFrame.Raised)
        self.size_grip.setObjectName("size_grip")
        self.horizontalLayout_7.addWidget(self.size_grip, 0, QtCore.Qt.AlignBottom)
        self.horizontalLayout_5.addWidget(self.frame_2)
        self.verticalLayout.addWidget(self.footer_frame, 0, QtCore.Qt.AlignBottom)
        MainWindow.setCentralWidget(self.centralwidget)

        self.retranslateUi(MainWindow)
        self.stackedWidget.setCurrentIndex(5)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "MainWindow"))
        self.open_close_side_bar_btn.setText(_translate("MainWindow", "Menu"))
        self.label_2.setText(_translate("MainWindow", "PSUTIL MANAGER"))
        self.label_4.setText(_translate("MainWindow", "CPU"))
        self.label_5.setText(_translate("MainWindow", "Battery"))
        self.label_6.setText(_translate("MainWindow", "System Information"))
        self.label_7.setText(_translate("MainWindow", "Activities"))
        self.label_8.setText(_translate("MainWindow", "Storage"))
        self.label_9.setText(_translate("MainWindow", "Sensors"))
        self.label_10.setText(_translate("MainWindow", "Networks"))
        self.label_16.setText(_translate("MainWindow", "CPU Count"))
        self.cpu_count.setText(_translate("MainWindow", "N/A"))
        self.cpu_per.setText(_translate("MainWindow", "N/A"))
        self.label_20.setText(_translate("MainWindow", "CPU Main Core"))
        self.cpu_main_core.setText(_translate("MainWindow", "N/A"))
        self.label_18.setText(_translate("MainWindow", "CPU Per"))
        self.label_26.setText(_translate("MainWindow", "Used RAM"))
        self.available_ram.setText(_translate("MainWindow", "N/A"))
        self.used_ram.setText(_translate("MainWindow", "N/A"))
        self.label_24.setText(_translate("MainWindow", "Available RAM"))
        self.total_ram.setText(_translate("MainWindow", "N/A"))
        self.label_22.setText(_translate("MainWindow", "Total RAM"))
        self.free_ram.setText(_translate("MainWindow", "N/A"))
        self.label_30.setText(_translate("MainWindow", "RAM Usage"))
        self.ram_usage.setText(_translate("MainWindow", "N/A"))
        self.label_28.setText(_translate("MainWindow", "Free RAM"))
        self.label_32.setText(_translate("MainWindow", "Battery Information"))
        self.label_33.setText(_translate("MainWindow", "Status"))
        self.battery_status.setText(_translate("MainWindow", "N/A"))
        self.label_34.setText(_translate("MainWindow", "Charge"))
        self.battery_charge.setText(_translate("MainWindow", "N/A"))
        self.label_35.setText(_translate("MainWindow", "Time Left"))
        self.battery_time_left.setText(_translate("MainWindow", "N/A"))
        self.label_36.setText(_translate("MainWindow", "Plugged In"))
        self.battery_plugged.setText(_translate("MainWindow", "N/A"))
        self.label_55.setText(_translate("MainWindow", "Activities"))
        self.activity_search.setPlaceholderText(_translate("MainWindow", "Search Processes"))
        item = self.tableWidget.horizontalHeaderItem(0)
        item.setText(_translate("MainWindow", "Process ID"))
        item = self.tableWidget.horizontalHeaderItem(1)
        item.setText(_translate("MainWindow", "Process Name"))
        item = self.tableWidget.horizontalHeaderItem(2)
        item.setText(_translate("MainWindow", "Process Status"))
        item = self.tableWidget.horizontalHeaderItem(3)
        item.setText(_translate("MainWindow", "Started"))
        item = self.tableWidget.horizontalHeaderItem(4)
        item.setText(_translate("MainWindow", "Suspend"))
        item = self.tableWidget.horizontalHeaderItem(5)
        item.setText(_translate("MainWindow", "Resume"))
        item = self.tableWidget.horizontalHeaderItem(6)
        item.setText(_translate("MainWindow", "Terminate"))
        item = self.tableWidget.horizontalHeaderItem(7)
        item.setText(_translate("MainWindow", "Kill"))
        self.pushButton_7.setText(_translate("MainWindow", "Suspend"))
        self.pushButton_8.setText(_translate("MainWindow", "Resume"))
        self.pushButton_9.setText(_translate("MainWindow", "Terminate"))
        self.pushButton_10.setText(_translate("MainWindow", "Kill"))
        self.label_56.setText(_translate("MainWindow", "Disk Partition"))
        item = self.storageTable.horizontalHeaderItem(0)
        item.setText(_translate("MainWindow", "Device"))
        item = self.storageTable.horizontalHeaderItem(1)
        item.setText(_translate("MainWindow", "Mount Point"))
        item = self.storageTable.horizontalHeaderItem(2)
        item.setText(_translate("MainWindow", "OPTS"))
        item = self.storageTable.horizontalHeaderItem(3)
        item.setText(_translate("MainWindow", "Max File"))
        item = self.storageTable.horizontalHeaderItem(4)
        item.setText(_translate("MainWindow", "Max Path"))
        item = self.storageTable.horizontalHeaderItem(5)
        item.setText(_translate("MainWindow", "Total Storage"))
        item = self.storageTable.horizontalHeaderItem(6)
        item.setText(_translate("MainWindow", "Used Storage"))
        item = self.storageTable.horizontalHeaderItem(7)
        item.setText(_translate("MainWindow", "Free Storage"))
        self.label_57.setText(_translate("MainWindow", "Sensors"))
        item = self.sensorTable.horizontalHeaderItem(0)
        item.setText(_translate("MainWindow", "Sensor"))
        item = self.sensorTable.horizontalHeaderItem(1)
        item.setText(_translate("MainWindow", "Label"))
        item = self.sensorTable.horizontalHeaderItem(2)
        item.setText(_translate("MainWindow", "Current"))
        item = self.sensorTable.horizontalHeaderItem(3)
        item.setText(_translate("MainWindow", "High"))
        item = self.sensorTable.horizontalHeaderItem(4)
        item.setText(_translate("MainWindow", "Critical"))
        self.label_58.setText(_translate("MainWindow", "Stats"))
        item = self.net_stats_table.horizontalHeaderItem(1)
        item.setText(_translate("MainWindow", "ISUP"))
        item = self.net_stats_table.horizontalHeaderItem(2)
        item.setText(_translate("MainWindow", "Duplex"))
        item = self.net_stats_table.horizontalHeaderItem(3)
        item.setText(_translate("MainWindow", "Speed"))
        item = self.net_stats_table.horizontalHeaderItem(4)
        item.setText(_translate("MainWindow", "MTU"))
        self.label_59.setText(_translate("MainWindow", "Network IO Counter"))
        item = self.net_io_table.horizontalHeaderItem(1)
        item.setText(_translate("MainWindow", "IO"))
        item = self.net_io_table.horizontalHeaderItem(2)
        item.setText(_translate("MainWindow", "BYTES SENT"))
        item = self.net_io_table.horizontalHeaderItem(3)
        item.setText(_translate("MainWindow", "BYTES RECEIVED"))
        item = self.net_io_table.horizontalHeaderItem(4)
        item.setText(_translate("MainWindow", "PACKETS SENT"))
        item = self.net_io_table.horizontalHeaderItem(5)
        item.setText(_translate("MainWindow", "PACKETS RECEIVED"))
        item = self.net_io_table.horizontalHeaderItem(6)
        item.setText(_translate("MainWindow", "ERR IN"))
        item = self.net_io_table.horizontalHeaderItem(7)
        item.setText(_translate("MainWindow", "ERR OUT"))
        item = self.net_io_table.horizontalHeaderItem(8)
        item.setText(_translate("MainWindow", "DROP OUT"))
        self.label_60.setText(_translate("MainWindow", "Network Addresses"))
        item = self.net_addresses_table.horizontalHeaderItem(1)
        item.setText(_translate("MainWindow", "FAMILY"))
        item = self.net_addresses_table.horizontalHeaderItem(2)
        item.setText(_translate("MainWindow", "ADDRESSES"))
        item = self.net_addresses_table.horizontalHeaderItem(3)
        item.setText(_translate("MainWindow", "NETMASK"))
        item = self.net_addresses_table.horizontalHeaderItem(4)
        item.setText(_translate("MainWindow", "BROADCAST"))
        item = self.net_addresses_table.horizontalHeaderItem(5)
        item.setText(_translate("MainWindow", "PTP"))
        self.label_61.setText(_translate("MainWindow", "Network Connections"))
        item = self.net_connections_table.horizontalHeaderItem(0)
        item.setText(_translate("MainWindow", "FD"))
        item = self.net_connections_table.horizontalHeaderItem(1)
        item.setText(_translate("MainWindow", "FAMILY"))
        item = self.net_connections_table.horizontalHeaderItem(2)
        item.setText(_translate("MainWindow", "FAMILY"))
        item = self.net_connections_table.horizontalHeaderItem(3)
        item.setText(_translate("MainWindow", "RADDR"))
        item = self.net_connections_table.horizontalHeaderItem(4)
        item.setText(_translate("MainWindow", "STATUS"))
        item = self.net_connections_table.horizontalHeaderItem(5)
        item.setText(_translate("MainWindow", "PID"))
        self.label_44.setText(_translate("MainWindow", "Version"))
        self.label_43.setText(_translate("MainWindow", "Platform"))
        self.label_41.setText(_translate("MainWindow", "System Information"))
        self.system_processor.setText(_translate("MainWindow", "N/A"))
        self.system_platform.setText(_translate("MainWindow", "N/A"))
        self.system_version.setText(_translate("MainWindow", "N/A"))
        self.label_45.setText(_translate("MainWindow", "System Date"))
        self.system_date.setText(_translate("MainWindow", "N/A"))
        self.label_51.setText(_translate("MainWindow", "System Time"))
        self.label_49.setText(_translate("MainWindow", "Processor"))
        self.label_50.setText(_translate("MainWindow", "Machine"))
        self.system_time.setText(_translate("MainWindow", "N/A"))
        self.system_machine.setText(_translate("MainWindow", "N/A"))
        self.system_system.setText(_translate("MainWindow", "N/A"))
        self.label_11.setText(_translate("MainWindow", "About"))
        self.label_12.setText(_translate("MainWindow", "App Designed By Collo for learning Perposes"))
        self.label_13.setText(_translate("MainWindow", "youtube"))
        self.label_14.setText(_translate("MainWindow", "@meTrying"))
        self.label_15.setText(_translate("MainWindow", "www.www.com"))
        self.label_3.setText(_translate("MainWindow", "Version 1.0 | Copyright @collo"))
        self.pushButton_2.setText(_translate("MainWindow", "?"))
import icons_rc
